<template>
  <div class="app">
    ...
    <div class="version-tag" v-if="status.latest || status.local || checking || githubOk !== null">
      ...
      <span v-if="status.latest" class="ver latest">最新 {{ status.latest }}</span>
      <template v-if="status.latest && status.local">
        <span class="ver-arrow">→</span>
        <span class="ver local">本地 {{ status.local }}</span>
      </template>
      <span v-else-if="status.latest && !status.local" class="ver none">未安装</span>
      <span v-else-if="checking" class="ver checking">检测中...</span>
      <button class="btn-refresh" @click="refreshVersion" :disabled="checking || busy" title="重新检测版本">
      ...
      <div class="modal-overlay" v-if="showSettings" @click.self="showSettings = false">
        <div class="modal">
          ...
          <div class="modal-body">
            ...
            <label class="field">
              <span class="field-label">Rocom Helper 存放父目录（自动创建 RocomHelper 子文件夹）</span>
              <div class="path-row">
...
async function checkVersion() {
  ...
  try {
    ...
    if (res.local) {
      addLog("info", `本地版本: ${res.local}`);
    }
    if (res.latest) {
      addLog("success", `最新版本: ${res.latest}`);
    }
    if (res.latest && !res.local) {
      addLog("warning", "本地未安装，需要下载");
    } else if (res.latest && res.latest !== res.local) {
      addLog("warning", `有新版本可用: ${res.latest}`);
    } else if (res.latest && res.latest === res.local) {
...
async function redownload() {
  ...
  if (status.local) {
    const confirmed = await ask(
      `本地已存在 Rocom Helper (${status.local})，是否删除并重新下载？`,
