...
fn get_store_dir(settings: &Settings) -> PathBuf {
    // Always append a "RocomHelper" subfolder to prevent accidental deletion
    // of the launcher itself when the user sets the path to the same directory
    let p = PathBuf::from(&settings.store_path).join("RocomHelper");
